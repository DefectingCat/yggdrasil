...
GitHub Actions (`.github/workflows/ci.yml`), five jobs following the 2026 native multi-runner pattern (zero QEMU): `test` (cargo fmt/clippy + wasm32 target check + Biome lint on `libs/` + `cargo test --all-features`) → `build-amd64` (`make docker-amd64`, native plain Dockerfile on the x86_64 runner) + `build-arm64` (native `ubuntu-24.04-arm` ARM64 runner, plain Dockerfile builds `aarch64-unknown-linux-musl` natively — 4-8× faster than QEMU). `test`+`build-amd64` run on **every push and PR**; `build-arm64`/`release`/`deploy` run **only on tag push (`v*`) or `workflow_dispatch`**. `release` downloads artifacts from both builds, extracts the x86_64 musl binary + `public/` from the amd64 image (via `docker create`+`docker cp` on the scratch image), and creates a GitHub Release with three assets: `yggdrasil-amd64.tar.gz` (x86 docker image), `yggdrasil-arm64.tar.gz` (arm docker image), `yggdrasil-x86_64-musl.tar.gz` (static musl server binary + public assets); release body is the matching `CHANGELOG.md` version section. `deploy` scp's the amd64 image to xun, `docker load` + `docker compose up -d app` (rolling restart, postgres/data volumes untouched), then polls `/readyz` via the `nginx-proxy` container — mirrors `scripts/xun.fish`. Required GitHub secrets for `deploy`: `XUN_SSH_KEY` (private key), `XUN_HOST`, `XUN_USER`; optional `XUN_PORT`, `XUN_DEPLOY_DIR` (default `/root/docker/yggdrasil`), `XUN_KNOWN_HOSTS` (server fingerprint — recommended over the `ssh-keyscan` TOFU fallback). The deploy key's public half goes in xun's `~/.ssh/authorized_keys`.
