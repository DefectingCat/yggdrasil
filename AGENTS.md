...
GitHub Actions (`.github/workflows/ci.yml`), three jobs: `test` (cargo fmt/clippy + wasm32 target check + Biome lint on `libs/` + `cargo test --all-features`) → `build` (`make docker-amd64`, native plain Dockerfile on the x86_64 runner — no QEMU/zig, host needs only Docker). `test`+`build` run on **every push (any branch or tag) and PR**. `deploy` runs **only on new tag push (`v*`) or manual `workflow_dispatch`** (branch pushes/PRs build+test but never deploy): scp the gzipped amd64 image artifact to xun, `docker load` + `docker compose up -d app` (rolling restart, postgres/data volumes untouched), then poll `/readyz` via the `nginx-proxy` container. Deployment mirrors `scripts/xun.fish` (app image only; the 5 runner sandbox images change rarely and are rebuilt/deployed manually). Required GitHub secrets for `deploy`: `XUN_SSH_KEY` (private key), `XUN_HOST`, `XUN_USER`; optional `XUN_PORT`, `XUN_DEPLOY_DIR` (default `/root/docker/yggdrasil`), `XUN_KNOWN_HOSTS` (server fingerprint — recommended over the `ssh-keyscan` TOFU fallback). The deploy key's public half goes in xun's `~/.ssh/authorized_keys`.
